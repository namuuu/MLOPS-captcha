...
main_path = __file__.replace("model_test.py", "").replace("OCR\\", "")
#print("main_path :", main_path)

TabImg = os.listdir(main_path+"/img")
...
# image = Image.open(main_path+"/img/4nRBYX.png").convert("RGB")
...
for i in range(10):
    image = Image.open(main_path+"/img/"+TabImg[i]).convert("RGB")
    pixel_values = processor(images=image, return_tensors="pt").pixel_values
    generated_ids = model.generate(pixel_values)
    text = processor.batch_decode(generated_ids, skip_special_tokens=True)[0]
